...
[선택지가 많아질수록 복잡성이 증가하고, 결정을 내리는 데 시간이 더 걸리게 됩니다](https://lawsofux.com/hicks-law/). 추가적인 버튼은 선택지를 늘리고, 결과적으로 시간을 더 소모하게 만듭니다.

또한, 키보드 사용자의 경우 [폼 제출을 위해 Enter 키를 눌렀을 때 어떤 버튼이 실행될지 명확하지 않아 혼란을 겪을 수 있습니다](https://adamsilver.io/blog/forms-with-multiple-submit-buttons-are-problematic/).

그렇다고 여러개의 버튼이 항상 불필요한 것은 아닙니다. 상황에 따라 여러개의 버튼이 필요한 경우도 있습니다.

버튼의 역할이 무엇인지 잘 생각한다면, 각 버튼을 어디에 배치해야 할지 더 쉽게 결정할 수 있습니다.

세 가지 예시를 통해 각각 상황에 맞는 버튼 배치 방법을 살펴보겠습니다.
...
Luke Wroblewski의 연구에 따르면, 취소 버튼은 주요 버튼의 오른쪽에 배치하고, 시각적으로 덜 눈에 띄도록 링크 형태로 스타일링하는 것이 좋다고 합니다.

하지만 취소 버튼을 주요 버튼 아래에 배치하는 것도 몇 가지 장점이 있습니다:
...
- 둘째, 앞서 뒤로 가기 버튼과 추가 버튼 섹션에서 설명한 대로, 취소 버튼은 폼의 주요 작업과 직접적인 관련이 없기 때문에 주요 버튼 아래에 배치하는 것이 더 적절합니다.
- 마지막으로, 성향이 다른 버튼들을 나란히 배치할 새로운 공간을 확보할 수 있습니다. 만약 모든 버튼들을 한 줄에 나열하면 사용자는 주요 버튼이 무엇인지 식별하기 어려울 수 있습니다.

### 2. 주요 버튼 위에 '추가하기' 버튼을 배치하세요
...
- 복제 버튼은 복제되는 필드 근처에 배치해, 사용자가 그 기능을 직관적으로 이해할 수 있게 합니다.- Erik Kennedy의 [근접성의 3가지 법칙](https://www.learnui.design/blog/the-3-laws-of-locality.html#1-put-the-control-where-it-affects-change)

### 3. 주요 버튼 옆에 '저장 후 종료' 버튼을 배치하세요

<figure>
<img src="/assets/images/where-to-put-buttons-on-forms/save-and-exit-button.png" alt="Save and exit 버튼 위치 비교 이미지" />
<figcaption>왼쪽: '저장 후 종료' 버튼이 주요 버튼 위에 있는 경우 (비추천). 오른쪽: '저장 후 종료' 버튼이 주요 버튼 옆에 있는 경우 (추천).</figcaption>
</figure>

사용자가 많은 양의 폼을 작성할 때 [진행 상황을 저장](https://github.com/alphagov/govuk-design-system-backlog/issues/87)해야 할 경우가 있습니다.

'저장 후 종료' 버튼을 주요 버튼 위에 배치하면, 이 버튼이 더 중요한 버튼이라는 잘못된 인식을 줄 수 있습니다.

아래에 배치하면 여러 버튼들이 쌓여 복잡해지고, '취소' 버튼이 있어야 할 자리를 차지하게 될 수 있습니다.

따라서 '저장 후 종료' 버튼을 주요 버튼 옆에 배치하는 것이 적절합니다. 이는 폼의 주요 작업과 직접적으로 관련된 동작이기 때문입니다.
